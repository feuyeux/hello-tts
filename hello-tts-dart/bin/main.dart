...
Future<void> displayVoicesByLanguage(TTSProcessor client, {String? filterLanguage}) async {
  try {
    ...
    if (filterLanguage != null) {
      if (voicesByLanguage.containsKey(filterLanguage)) {
        ...
        logger.info('${filterLanguage.toUpperCase()} Voices (${langVoices.length} voices):');

        // Show first 5 voices
        for (final voice in langVoices.take(5)) {
          logger.info('  ${voice.name} - ${voice.displayName} (${voice.gender})');
        }
...
Future<void> main(List<String> arguments) async {
  final parser = ArgParser()
    ...
    ..addOption('backend', abbr: 'b', help: 'TTS backend to use', defaultsTo: 'edge', allowed: ['edge', 'google'])
  ...
  try {
    ...
    rootLogger.onRecord.listen((record) {
      ...
      final ts = DateTime.fromMillisecondsSinceEpoch(record.time.millisecondsSinceEpoch, isUtc: true).toIso8601String();
      stderr.writeln('$ts [${record.level.name}] ${record.loggerName}: ${record.message}');
    });
    ...
    if (results['help'] as bool) {
      ...
      helpLogger.info('  dart run bin/main.dart --backend edge --text "Hello World" --voice "en-US-JennyNeural"');
      helpLogger.info('  dart run bin/main.dart --backend edge --text "你好世界" --voice "zh-CN-XiaoxiaoNeural"');
      helpLogger.info('  # Google TTS examples');
      helpLogger.info('  dart run bin/main.dart --backend google --text "Hello World" --voice "en"');
      helpLogger.info('  dart run bin/main.dart --backend google --text "你好世界" --voice "zh"');
  ...
  logger.info('Text: $text');
      ...
      final audioData = await client.synthesizeText(text, voice, format: format);
  ...
  logger.info('📊 File size: ${(audioData.length / 1024).toStringAsFixed(1)} KB');
