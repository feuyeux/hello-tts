...
Future<List<LanguageConfig>> loadLanguageConfig(String backend) async {
  ...
  final perBackendPath = (backend.toLowerCase() == 'google') ? '../shared/google_tts_voices.json' : '../shared/edge_tts_voices.json';
  ...
  try {
    ...
    if (await File(unifiedPath).exists()) {
      ...
      return languagesJson.map((v) {
        final map = v as Map<String, dynamic>;
        final voice = (backend.toLowerCase() == 'google') ? (map['google_voice'] ?? map['voice'] ?? '') : (map['edge_voice'] ?? map['voice'] ?? '');
        return LanguageConfig(
          code: map['code'] ?? '',
          name: map['name'] ?? '',
          flag: map['flag'] ?? '',
      ...
      }).where((lc) => lc.code.isNotEmpty).toList();
    } else if (await File(perBackendPath).exists()) {
      file = File(perBackendPath);
      final content = await file.readAsString();
      final Map<String, dynamic> config = json.decode(content);
      final List<dynamic> languagesJson = config['languages'] ?? config['voices'] ?? [];
      return languagesJson
          .map((lang) {
            final m = lang as Map<String, dynamic>;
            return LanguageConfig(
              code: m['code'] ?? '',
              name: m['name'] ?? '',
              flag: m['flag'] ?? '',
              text: m['text'] ?? '',
              voice: m['voice'] ?? m['google_voice'] ?? m['edge_voice'] ?? '',
              altVoice: m['alt_voice'] ?? '',
            );
          })
          .where((lc) => lc.code.isNotEmpty)
          .toList();
    } else {
      logger.severe('❌ Configuration file not found (tried $unifiedPath and $perBackendPath)');
      return [];
    }
  } catch (e) {
...
Future<bool> generateAudioForLanguage(
  ...
  final backend = Platform.environment['TTS_BACKEND'] ?? 'edge';
...
Future<void> main(List<String> args) async {
  ...
  logger.info('🌍 Multilingual Edge TTS Demo - Dart Implementation');
  logger.info('=' * 60);
  logger.info('Backend: $backend');
  logger.info('Generating audio for 12 languages with custom sentences...');

  // Load language configuration
  final languages = await loadLanguageConfig(backend);
  if (languages.isEmpty) {
    logger.severe('❌ Failed to load language configuration or no languages found');
    exit(1);
  }
  ...
  logger.info('⏱️  Total time: ${duration.inSeconds}.${duration.inMilliseconds % 1000} seconds');
  logger.info('📁 Output files saved in: $outputPath');

  if (successfulCount > 0) {
    logger.info('\n🎉 Successfully generated audio files for $successfulCount languages!');
    logger.info('You can find all generated MP3 files in the output directory.');
  }
